...
public class GeneratorScreen extends ContainerScreen<GeneratorContainer> {
    ...
    protected void drawBackground(float delta, int mouseX, int mouseY) {
        ...
        int chargeRatio = (int) Math.round((container.getBlockEntity().getStored(null) / container.getBlockEntity().getMaxStoredPower()) * 28);
        this.blit(this.x + 30, this.y + 16 + 30 - chargeRatio, 176, 42 - chargeRatio, 15, chargeRatio + 1);
    ...
    protected void drawForeground(int mouseX, int mouseY) {
        ...
        if (mouseX - this.x > 28 && mouseX - this.x < 46 && mouseY - this.y > 16 && mouseY - this.y < 50) {
            renderTooltip(String.valueOf(container.getBlockEntity().getStored(null)), mouseX - this.x, mouseY - this.y);
        }
    }
}
