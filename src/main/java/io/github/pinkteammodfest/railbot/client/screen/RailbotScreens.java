...
public class RailbotScreens {
    ...
    public static void init() {
        ScreenProviderRegistry.INSTANCE.registerFactory(Railbot.id(GENERATOR_ID), (syncId, identifier, player, buf) -> {
            BlockEntity blockEntity = player.world.getBlockEntity(buf.readBlockPos());
            if(blockEntity instanceof GeneratorBlockEntity) {
                return new GeneratorScreen(new GeneratorContainer(syncId, player.inventory, (GeneratorBlockEntity) blockEntity), player.inventory);
            } else { // should only occur on malformed packets
                return new GeneratorScreen(new GeneratorContainer(syncId, player.inventory, new GeneratorBlockEntity()), player.inventory);
            }
        });
    }
